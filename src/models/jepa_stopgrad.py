"""Stop-gradient JEPA orchestrator.

Composes a shared EncounterEncoder and Predictor. The target path
blocks gradient flow via torch.no_grad() + detach().
"""
...
class JEPAStopGrad(nn.Module):
    """Joint Embedding Predictive Architecture — stop-gradient variant.

    Both context and target paths share the same encoder; the target
